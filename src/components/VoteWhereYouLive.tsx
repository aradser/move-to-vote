...
function Unit({ n, l }: { n: number | null; l: string }) {
  return (
    <span className="flex flex-col items-center min-w-[2.5rem]">
      <span className="tabular-nums font-black text-lg sm:text-xl leading-none" style={{ fontVariantNumeric: "tabular-nums" }}>
        {n === null ? "--" : String(n).padStart(2, "0")}
      </span>
      <span className="text-[10px] sm:text-xs opacity-60 font-semibold mt-0.5">{l}</span>
    </span>
  );
}
...
function CountdownBlock({ t, cd }: { t: Copy; cd: ReturnType<typeof useCountdown> }) {
  return (
    <div className="flex flex-col items-center gap-3">
      ...
      <div dir="ltr" className="flex items-start gap-3 sm:gap-4 bg-zinc-950 text-white rounded-2xl shadow-lg px-6 py-3">
